...
const scenes = ['intro', 'constellation', 'book', 'universe', 'letter', 'time', 'corner', 'finale'];
...
/* ---------- Scene 8: the last page ---------- */
// Fill in your own closing message below.
const FINAL_MESSAGE = `[ Your final personal message goes here — whatever you want to say last. ]`;
...
function runFinale() {
  ...
  message.textContent = FINAL_MESSAGE;

  setTimeout(() => finaleScene.classList.add('pulled-back'), 400);
  setTimeout(() => line1.classList.add('show'), 2600);
  setTimeout(() => line2.classList.add('show'), 4600);
  setTimeout(() => message.classList.add('show'), 6800);
  setTimeout(() => line3.classList.add('show'), 9200);
  // everything fades except the ambient stars — a quiet ending
  setTimeout(() => content.classList.add('fade-out'), 13000);
}
